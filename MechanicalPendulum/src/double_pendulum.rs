...
impl DoublePendulumModel {
    ...
    fn f2(&self, theta1: f64, omega1: f64, theta2: f64, omega2: f64) -> f64 {
        ...
        let t21s = t21.sin();
        let a = self.mass * t21s * (LENGTH * omega2.powi(2) + theta2.cos()) + self.mass * (2.0 * t21).sin() * omega1.powi(2) / 2.0 - theta1.sin();
    ...
    fn f4(&self, theta1: f64, omega1: f64, theta2: f64, omega2: f64) -> f64 {
        let t21 = theta2 - theta1;
        -(theta2.sin() + t21.sin() * omega1.powi(2) + t21.cos() * self.f2(theta1, omega1, theta2, omega2)) / self.length
    }

    fn f(&self, theta1: f64, omega1: f64, theta2: f64, omega2: f64) -> (f64, f64, f64, f64) {
        (DoublePendulumModel::f1(omega1), self.f2(theta1, omega1, theta2, omega2),
         DoublePendulumModel::f3(omega2), self.f4(theta1, omega1, theta2, omega2))
    }
}
...
impl PendulumModel for DoublePendulumModel {
    ...
    fn step(&mut self) {
        let (k1, l1, m1, n1) = self.f(
            self.theta1, self.omega1, self.theta2, self.omega2);
        let (k2, l2, m2, n2) = self.f(
            self.theta1 + k1 * self.dtime2, self.omega1 + l1 * self.dtime2,
            self.theta2 + m1 * self.dtime2, self.omega2 + n1 * self.dtime2);
        let (k3, l3, m3, n3) = self.f(
            self.theta1 + k2 * self.dtime2, self.omega1 + l2 * self.dtime2,
            self.theta2 + m2 * self.dtime2, self.omega2 + n2 * self.dtime2);
        let (k4, l4, m4, n4) = self.f(
            self.theta1 + k3 * self.dtime, self.omega1 + l3 * self.dtime,
            self.theta2 + m3 * self.dtime, self.omega2 + n3 * self.dtime);
